...
- **双模式支持**：V2 完整版（增量式上下文迭代）+ Lite 简化版（并行工具调用），满足不同场景需求
...
- **Agent 层**：
  ...
  - **Lite 简化版**：`LiteDispatcher` 并行工具调用，Plan → Execute → Answer
- **工具层**：
  - `search`：知识库检索（最多10次）
...
- API Key：阿里云百炼、SiliconFlow、Metaso 搜索（按需配置）

> ⚠️ 请在项目根目录创建并使用 `venv` 虚拟环境。
...
# 白名单配置
PYTHON_ALLOWED_MODULES = {...}  # 详见 config.py
PYTHON_ALLOWED_BUILTINS = [...]  # 详见 config.py
```
...
1. **单知识库查询**：V2 架构暂不支持跨知识库检索（计划中）
2. **文件大小限制**：单文件最大 16MB（可配置）
3. **网络搜索次数**：单次对话最多 3 次（防止滥用）
4. **执行器内存**：HSL 执行器限制 256MB（可配置）
